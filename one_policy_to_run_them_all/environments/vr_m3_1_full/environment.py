...
from one_policy_to_run_them_all.environments.vr_m3_1_full.viewer import MujocoViewer
from one_policy_to_run_them_all.environments.vr_m3_1_full.control_functions.handler import get_control_function
from one_policy_to_run_them_all.environments.vr_m3_1_full.command_functions.handler import get_command_function
from one_policy_to_run_them_all.environments.vr_m3_1_full.sampling_functions.handler import get_sampling_function
from one_policy_to_run_them_all.environments.vr_m3_1_full.initial_state_functions.handler import get_initial_state_function
from one_policy_to_run_them_all.environments.vr_m3_1_full.reward_functions.handler import get_reward_function
from one_policy_to_run_them_all.environments.vr_m3_1_full.termination_functions.handler import get_termination_function
from one_policy_to_run_them_all.environments.vr_m3_1_full.domain_randomization.action_delay_functions.handler import get_get_domain_randomization_action_delay_function
from one_policy_to_run_them_all.environments.vr_m3_1_full.domain_randomization.mujoco_model_functions.handler import get_domain_randomization_mujoco_model_function
from one_policy_to_run_them_all.environments.vr_m3_1_full.domain_randomization.seen_robot_functions.handler import get_domain_randomization_seen_robot_function
from one_policy_to_run_them_all.environments.vr_m3_1_full.domain_randomization.unseen_robot_functions.handler import get_domain_randomization_unseen_robot_function
from one_policy_to_run_them_all.environments.vr_m3_1_full.domain_randomization.perturbation_functions.handler import get_domain_randomization_perturbation_function
from one_policy_to_run_them_all.environments.vr_m3_1_full.observation_noise_functions.handler import get_observation_noise_function
from one_policy_to_run_them_all.environments.vr_m3_1_full.observation_dropout_functions.handler import get_observation_dropout_function
from one_policy_to_run_them_all.environments.vr_m3_1_full.terrain_functions.handler import get_terrain_function


class VRM31Full(gym.Env):
    JOINT_NAMES = [
    ...
    EFFORT_LIMITS = [360, 360, 130, 360, 120, 120] * 2 + [102] + [66, 66, 34, 34, 11, 11, 11] * 2
    LONG_NAME = "vr_m3_1_full"
    SHORT_NAME = "vrm3_full"
    ...
    def __init__(self, seed, render,
        ...
        self.nominal_joint_positions = np.array([
            -0.1, 0.0, 0.0, 0.2, -0.1, 0.0, -0.1, 0.0, 0.0, 0.2, -0.1, 0.0, 0.0,
            0.0, 0.15, 0.0, 1.57, 0.0, 0.0, 0.0, 0.0, -0.15, 0.0, 1.57, 0.0, 0.0, 0.0,
        ])
        self.max_joint_velocities = np.array([
            14.653, 14.653, 31.4, 14.653, 16.747, 16.747, 14.653, 14.653, 31.4, 14.653, 16.747, 16.747, 4.18,
            4.29, 4.29, 5.13, 5.13, 6.17, 6.17, 6.17, 4.29, 4.29, 5.13, 5.13, 6.17, 6.17, 6.17,
        ])
        self.initial_drop_height = 0.854
        ...
        if self.add_goal_arrow:
            ...
            xml_handle = mjcf.from_path(xml_path)
            for joint_name, effort_limit in zip(self.JOINT_NAMES, self.EFFORT_LIMITS):
                xml_handle.actuator.add("motor", name=joint_name.replace("_joint", "_motor"), joint=joint_name, gear=[1], ctrllimited=True, ctrlrange=[-effort_limit, effort_limit])
            self.model = mujoco.MjModel.from_xml_string(xml=xml_handle.to_xml_string(), assets=xml_handle.get_assets())
        self.model.opt.timestep = timestep
        self.model.dof_armature[6:] = np.array([0.1404, 0.1404, 0.02864, 0.1404, 0.03006, 0.03006] * 2 + [0.5804369] + [0.3896782, 0.3896782, 0.1142512, 0.1142512, 0.0836482, 0.0836482, 0.0836482] * 2)
        self.model.dof_frictionloss[6:] = np.array([0.1] * 12 + [18.0] + [3.0, 3.0, 3.0, 1.2, 1.3, 0.7, 0.7] * 2)
        self.data = mujoco.MjData(self.model)

        collision_groups = [("floor", ["floor"]),
                            ("left_foot", [f"left_ankle_roll_link_collision_{i}" for i in range(1, 11)]),
                            ("right_foot", [f"right_ankle_roll_link_collision_{i}" for i in range(1, 11)]),
                            ("trunk", ["pelvis_collision_1"])]
        ...
        self.joint_names = self.JOINT_NAMES.copy()
        self.joint_nr_direct_child_joints = [
            1, 1, 1, 1, 1, 0, 1, 1, 1, 1, 1, 0, 2,
    ...
    def step(self, action):
        ...
        if self.mode == "test":
            ...
            elif False and Path("commands.txt").is_file():
        ...
        action = np.pad(action, (0, self.model.nu - action.shape[0])) if action.shape[0] < self.model.nu else action
        ...
        if self.episode_step in (0, 50, 99, 100, 200, 300, 400, 498):
            print(f"VRM31Full motion step={self.episode_step} root={np.array2string(self.data.qpos[:3], precision=4)} legs={np.array2string(self.data.qpos[7:19], precision=3)}")
        ...
        if terminated or truncated:
            print(f"VRM31Full done step={self.episode_step} terminated={terminated} truncated={truncated} horizon={self.horizon} root_z={self.data.qpos[2]:.4f}")
    ...
    def get_name_to_description_vector(self):
        ...
        for foot_name in self.foot_names:
            foot = self.data.geom(foot_name.replace("foot", "ankle_roll_link_collision") + "_5")
        ...
        for i, joint_name in enumerate(self.joint_names):
            ...
            name_to_description_vector[joint_name] = np.concatenate([
                ...
                (np.array([self.control_function.p_gains[i], self.control_function.d_gains[i], self.control_function.scaling_factors[i]]) / [100.0 / 2, 2.0 / 2, 0.8 / 2]) - 1.0,
                (self.mass / (170.0 / 2)) - 1.0,
                (self.robot_dimensions / (2.0 / 2)) - 1.0
            ])
