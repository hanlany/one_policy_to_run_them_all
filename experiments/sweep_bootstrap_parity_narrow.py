...
DEFAULT_OUTPUT_DIR = ROOT / "experiments" / "teacher-student" / "bootstrap_sweep_narrow"

if str(STUDENT_DIR) not in sys.path:
    sys.path.append(str(STUDENT_DIR))

from bootstrap_backend import (  # noqa: E402
...
def format_trial_name(config: dict[str, object]) -> str:
    return (
        f"ro-{config['readout']}"
        f"_strategy-{config['input_strategy']}"
...
def main():
    parser = argparse.ArgumentParser(
        description="Run a narrow bootstrap parity sweep around the best region from the broad bootstrap search."
    )
    ...
    parser.add_argument("--readouts", default="mean,last", help="Comma-separated selected bootstrap readouts to train/evaluate.")
    parser.add_argument("--input-strategies", default="signed_split", help="Comma-separated bootstrap input strategies.")
    parser.add_argument("--input-weights", default="2.0", help="Comma-separated bootstrap input weight values.")
    parser.add_argument("--input-biases", default="0.0", help="Comma-separated bootstrap input bias values.")
    parser.add_argument("--neuron-thresholds", default="0.35,0.5,0.65", help="Comma-separated bootstrap neuron thresholds.")
    parser.add_argument("--current-decays", default="0.2,0.25,0.3", help="Comma-separated bootstrap current decays.")
    parser.add_argument("--voltage-decays", default="0.005,0.01,0.02", help="Comma-separated bootstrap voltage decays.")
    ...
    readouts = parse_choice_list(args.readouts, SUPPORTED_SNN_READOUTS, "readout")
                                ...
                                for timestep in timesteps:
                                    for num_sample_iter in num_sample_iters:
                                        for sample_period in sample_periods:
                                            ...
                                            print(
                                                f"[{trial_index}/{total}] "
                                                f"ro={readout} strategy={input_strategy} iw={input_weight:.3f} ib={input_bias:.3f} "
    ...
    print("best_narrow_bootstrap_configuration:")
    if best_result is None:
        print("  none")
    else:
        print(f"  readout: {best_result['readout']}")
    ...
    if best_beating_result is None:
        print("best_narrow_bootstrap_beating_conversion: none")
    else:
        print("best_narrow_bootstrap_beating_conversion:")
        print(f"  readout: {best_beating_result['readout']}")
