...
- Added distinct attributed `vr_m3_1_12dof` and `vr_m3_1_full` packages from the pinned Apache-2.0 source, plus separate registry entries, termination keys, recording choices, and named expert presets. Package cleanup and wheel verification remain.
- The 12-DoF adapter compiles and routes with `nq=19,nv=18,nu=12`, observation size 356; the full adapter compiles and routes with `nq=34,nv=33,nu=27`, observation size 746. Both focused constructor/first-step smokes are finite.
- Both named presets restore the unchanged checkpoint (SHA-256 `e851b6c4b2d1da55050d5ba1500cc893073795e9a4b83dad2abc6ce58e489c60`) and produced decodable 2-second MP4s at 640x480, 50 FPS, 100 frames under run `20260730T194945Z`; resolved configs are saved alongside them.
- Frame inspection caught and corrected an immediate 12-DoF backward fall by encoding the VinRobot per-joint P/D/action scales in the existing 23-field descriptions. Its corrected short video remains upright with visible motion; the full variant remains upright but develops backward lean by the final short frame, so neither short diagnostic is the final gate.
- Next action is embodiment-only rollout stabilization (especially full-body), then change both presets to the configured final interval, capture logs, rerun only failed recordings, validate first/middle/final frames, clean/package assets, and write the combined manifest. The combined terminal gate remains incomplete.
